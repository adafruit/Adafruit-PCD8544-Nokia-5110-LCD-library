...
public:
  PCD8544(byte SCLK, byte DIN, byte DC, byte CS=0, byte RST=0);

  void init(char contrast=50);
...
private:
  ...
  byte _din, _sclk, _dc, _rst, _cs;

  inline void spiwrite(byte c);
  void my_setpixel(byte x, byte y, byte color);
  void updateBoundingBox(byte xmin, byte ymin, byte xmax, byte ymax);
};
